...
| **CF-14** | **Row-provenance replay + silent proxy fallback.** Matrices carry no row ids; analyses replayed Phase-4 iteration and, on any mismatch, silently fell back to one-pseudo-chain-per-behaviour — under which the within-chain permutation is a **no-op** (null ≡ real, p≈1.0) while looking legitimate. Same replay fragility underlies CBS phases 10/12 row indexing | chain-stratified null (the primary defence for every claim); CBS trajectories | S0 | bug→**FIXED 2026-06-12** (`row_index.json` sidecar at extraction; `require_aligned` hard errors; vacuous-permutation guard raises in `src/nulls.py`) | sidecar exists only after re-extraction. CBS damage **empirically confirmed** (data-integrity review): the skip-blind replay mis-assigns **93.5%** of uncertainty-estimation and **79.0%** of example-testing rows for bounds-check-only consumers (10/12); 09's hard length check silently **drops** those cells instead. CBS consumers still to be wired to the sidecar (deferred with CBS) |
